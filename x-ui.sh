...
update() {
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/krisna112/panel-xui/main/update.sh)
    if [[ $? == 0 ]]; then
...
update_menu() {
    ...
    wget -O /usr/bin/x-ui https://raw.githubusercontent.com/krisna112/panel-xui/main/x-ui.sh
...
legacy_version() {
    ...
    install_command="bash <(curl -Ls \"https://raw.githubusercontent.com/krisna112/panel-xui/v$tag_version/install.sh\") v$tag_version"
...
update_from_repo() {
    confirm "This function will update the panel to the latest code from the repository. Ensure you have a stable internet connection. Do you want to continue?" "y"
    if [[ $? != 0 ]]; then
        LOGE "Cancelled"
        if [[ $# == 0 ]]; then
            before_show_menu
    ...
    if ! command -v go &> /dev/null || ! command -v git &> /dev/null; then
        ...
        case "${release}" in
            ...
            alpine)
                ...
                ;;
            *)
                echo "Unsupported OS for auto-installing Go/Git. Please install them manually."
                before_show_menu
                return 1
                ;;
        esac
    ...
    git clone https://github.com/krisna112/panel-xui.git "$BUILD_DIR"
    ...
    rm -rf "$BUILD_DIR"
    LOGI "Update from repository complete."
    before_show_menu
}
...
uninstall() {
    ...
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/krisna112/panel-xui/main/install.sh)${plain}"
    echo ""
...
update_shell() {
    wget -O /usr/bin/x-ui -N https://github.com/krisna112/panel-xui/raw/main/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
│  ${green}26.${plain} Update from Repository (Latest Code)      │
╚────────────────────────────────────────────────╝
"
    show_status
