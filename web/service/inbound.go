...
func (s *InboundService) updateClientTraffics(tx *gorm.DB, oldInbound *model.Inbound, newInbound *model.Inbound) error {
	...
	for _, oldClient := range oldClients {
		...
		if !emailExists {
			...
			// Update existing client stats (e.g. TotalDaily)
			err = s.UpdateClientStat(tx, oldClient.Email, &oldClient)
			if err != nil {
				return err
			}
		}
...
func (s *InboundService) AddTraffic(inboundTraffics []*xray.Traffic, clientTraffics []*xray.ClientTraffic) (error, bool) {
	...
	needRestart3, count, err := s.autoDeleteDepletedClients(tx)
	if err != nil {
		logger.Warning("Error in deleting depleted clients:", err)
	} else if count > 0 {
		logger.Debugf("%v depleted clients deleted", count)
	}
	...
	for dbTraffic_index := range dbClientTraffics {
		for traffic_index := range traffics {
			if dbClientTraffics[dbTraffic_index].Email == traffics[traffic_index].Email {
				...
				dbClientTraffics[dbTraffic_index].UpDaily += traffics[traffic_index].Up
				dbClientTraffics[dbTraffic_index].DownDaily += traffics[traffic_index].Down
		...
		for client_index := range clients {
			...
			for traffic_index, traffic := range traffics {
				if traffic.Email == c["email"].(string) {
					...
					traffics[traffic_index].DepletedAt = 0
					if !traffic.Enable {
...
func (s *InboundService) disableInvalidClients(tx *gorm.DB) (bool, int64, error) {
	...
	if p != nil {
		...
		err := tx.Table("inbounds").
			...
			Where("((client_traffics.total > 0 AND client_traffics.up + client_traffics.down >= client_traffics.total) OR (client_traffics.total_daily > 0 AND client_traffics.up_daily + client_traffics.down_daily >= client_traffics.total_daily) OR (client_traffics.expiry_time > 0 AND client_traffics.expiry_time <= ?)) AND client_traffics.enable = ?", now, true).
			Scan(&results).Error
		if err != nil {
			return false, 0, err
		}
	...
	result := tx.Model(xray.ClientTraffic{}).
		Where("((total > 0 and up + down >= total) or (total_daily > 0 and up_daily + down_daily >= total_daily) or (expiry_time > 0 and expiry_time <= ?)) and enable = ?", now, true).
		Updates(map[string]any{
			"enable":      false,
			"depleted_at": gorm.Expr("CASE WHEN depleted_at IS NULL OR depleted_at = 0 THEN ? ELSE depleted_at END", now),
		})
	err := result.Error
	count := result.RowsAffected
	return needRestart, count, err
...
// autoDeleteDepletedClients removes clients that have been disabled due to expiry or traffic limits
// after the configured grace period.
func (s *InboundService) autoDeleteDepletedClients(tx *gorm.DB) (bool, int64, error) {
	settingService := SettingService{}
	enabled, err := settingService.GetAutoDeleteDepletedClients()
	if err != nil {
		return false, 0, err
	}
	if !enabled {
	...
	graceDays, err := settingService.GetAutoDeleteDepletedClientsDays()
	if err != nil {
		return false, 0, err
	}
	if graceDays <= 0 {
	...
	threshold := time.Now().Add(-time.Duration(graceDays)*24*time.Hour).Unix() * 1000
	var traffics []xray.ClientTraffic
	if err := tx.Model(xray.ClientTraffic{}).
		Where("enable = ? AND depleted_at > 0 AND depleted_at <= ?", false, threshold).
	...
	for i := range traffics {
		...
		if err := tx.Where("id = ?", traffic.InboundId).First(&inbound).Error; err != nil {
			logger.Warningf("auto delete: unable to load inbound %d for client %s: %v", traffic.InboundId, traffic.Email, err)
			continue
		}

		var settings map[string]any
		if err := json.Unmarshal([]byte(inbound.Settings), &settings); err != nil {
			return needRestart, deleted, err
		}
		clients, ok := settings["clients"].([]any)
		if !ok {
			continue
		...
		for _, client := range clients {
			...
			if email, ok := c["email"].(string); ok && email == traffic.Email {
				found = true
				needApiDel, _ = c["enable"].(bool)
			} else {
				newClients = append(newClients, client)
		...
		if err := s.DelClientIPs(tx, traffic.Email); err != nil {
			return needRestart, deleted, err
		}
		if err := s.DelClientStat(tx, traffic.Email); err != nil {
			return needRestart, deleted, err
		}

		if needApiDel && p != nil {
			s.xrayApi.Init(p.GetAPIPort())
			if err1 := s.xrayApi.RemoveUser(inbound.Tag, traffic.Email); err1 == nil {
				logger.Debug("Client deleted by api:", traffic.Email)
			} else if !strings.Contains(err1.Error(), fmt.Sprintf("User %s not found.", traffic.Email)) {
				logger.Debug("Error in deleting client by api:", err1)
				needRestart = true
			}
			s.xrayApi.Close()
	...
	return needRestart, deleted, nil
}
...
func (s *InboundService) AddClientStat(tx *gorm.DB, inboundId int, client *model.Client) error {
	...
	clientTraffic.UpDaily = 0
	clientTraffic.DownDaily = 0
	clientTraffic.TotalDaily = client.LimitDaily
...
func (s *InboundService) UpdateClientStat(tx *gorm.DB, email string, client *model.Client) error {
	result := tx.Model(xray.ClientTraffic{}).
		...
		Updates(map[string]any{
			...
			"reset":       client.Reset,
			"total_daily": client.LimitDaily,
			"depleted_at": gorm.Expr("CASE WHEN ? = ? THEN 0 ELSE depleted_at END", client.Enable, true),
		})
	err := result.Error
	return err
}
...
func (s *InboundService) ResetClientTrafficByEmail(clientEmail string) error {
	...
	result := db.Model(xray.ClientTraffic{}).
		...
		Updates(map[string]any{"enable": true, "up": 0, "down": 0, "depleted_at": 0})
